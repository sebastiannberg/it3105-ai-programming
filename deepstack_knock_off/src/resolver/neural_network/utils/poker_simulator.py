...
def generate_turn_case():
    ...
    state, player_hand = simulate_poker_game("turn", poker_config, poker_rules)
    ...
    print(state)
    print(player_hand)
    _, _, v1, v2 = resolver.resolve(state, r1, r2, end_stage="river", end_depth=1, T=10, player_hand=player_hand)

    end_time = time.time()
    duration = end_time - start_time
    duration_minutes = duration / 60
    print(f"Generating case took {duration_minutes:.2f} minutes to run")
    return r1, r2, state.public_cards, state.pot, v1, v2

def generate_turn_cases(num_cases: int):
    start_time = time.time()

    date_str = datetime.datetime.now().strftime("%Y-%m-%d_%H-%M-%S")
    filename = f"turn_cases_{date_str}.csv"
    current_dir = os.path.dirname(os.path.abspath(__file__))
    parent_dir = os.path.dirname(current_dir)
    output_file = os.path.join(parent_dir, "data", "raw", filename)
    ...
    for i in range(num_cases):
        print(f"Case {i+1}")
        r1, r2, public_cards, pot, v1, v2 = generate_turn_case()
        # Convert data to a format that can be saved in CSV
        case_dict = {
            "case_id": i,
            "r1": r1.flatten().tolist(),
...
def generate_flop_case():
    ...
    state, player_hand = simulate_poker_game("flop", poker_config, poker_rules)
    ...
    print(state)
    print(player_hand)
    _, _, v1, v2 = resolver.resolve(state, r1, r2, end_stage="turn", end_depth=1, T=10, player_hand=player_hand)
...
def generate_flop_cases(num_cases: int):
    start_time = time.time()

    date_str = datetime.datetime.now().strftime("%Y-%m-%d_%H-%M-%S")
    filename = f"flop_cases_{date_str}.csv"
    current_dir = os.path.dirname(os.path.abspath(__file__))
    parent_dir = os.path.dirname(current_dir)
    output_file = os.path.join(parent_dir, "data", "raw", filename)
    ...
    for i in range(num_cases):
        print(f"Case {i+1}")
        r1, r2, public_cards, pot, v1, v2 = generate_flop_case()
        # Convert data to a format that can be saved in CSV
        case_dict = {
            "case_id": i,
            "r1": r1.flatten().tolist(),
...
generate_flop_cases(num_cases=5000)
