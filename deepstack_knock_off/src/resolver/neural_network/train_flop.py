...
from resolver.neural_network.models.flop_network import FlopNetwork
...
def train():
    ...
    BATCH_SIZE = 16
    CSV_FILENAME = "flop_cases_2024-04-25_18-02-15.csv"
    NORMALIZATION_PARAMS_PATH = os.path.join(os.path.dirname(__file__), "data", "flop_normalization_params.json")
    SAVED_MODELS_PATH = os.path.join(os.path.dirname(__file__), "saved_models")

    print("Creating datasets")
    train_dataset = PokerDataset(stage= "flop", csv_filename=CSV_FILENAME, mode="train", param_file=NORMALIZATION_PARAMS_PATH)
    validation_dataset = PokerDataset(stage="flop", csv_filename=CSV_FILENAME, mode="validation", param_file=NORMALIZATION_PARAMS_PATH)
    ...
    for epoch in range(NUM_EPOCHS):
        ...
        if average_validation_loss < best_validation_loss and (epoch + 1) % 5 == 0:
            ...
            model_save_path = os.path.join(SAVED_MODELS_PATH, "flop", f"{current_time}_epoch_{epoch+1}.pt")
...
def test(model_filename: str):
    ...
    test_dataset = PokerDataset(stage="flop", csv_filename=CSV_FILENAME, mode="test", param_file=NORMALIZATION_PARAMS_PATH)
    print(f"Test dataset created with {len(test_dataset)} samples")

    print("Creating data loaders")
    test_data_loader = DataLoader(test_dataset, batch_size=BATCH_SIZE, shuffle=False)

    model_path = os.path.join(SAVED_MODELS_PATH, "flop", model_filename)
    model = FlopNetwork()
